/* FISZKI-QUIZ – wpisywanie odpowiedzi + łagodne porównanie + Import/Export */
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------- SRS ---------- */
  const STORAGE_KEY = 'flash_srs_v1'; // wspólna baza
  const MAX_BOX = 5;
  ...
  function typoThreshold(len){ if(len<=5) return 1; if(len<=10) return 2; return 3; }
  function similarEnough(userRaw, expectedRaw){
    const u=canon(userRaw), e=canon(expectedRaw); if(!u&&!e) return true; if(u===e) return true;
    return editDistance(u,e) <= typoThreshold(Math.max(u.length,e.length));
  }
  ...
  function promote(card){ const all=readAll(); const idx=findIndex(all,card); if(idx<0) return; const e=all[idx]; e.box=Math.min(MAX_BOX,(e.box||1)+1); e.dueAt=Date.now()+INTERVALS_DAYS[e.box]*DAY_MS; writeAll(all); }
  function demote(card){ const {arr, idx}=ensureEntry(card); const e=arr[idx]; e.box=1; e.dueAt=Date.now()+INTERVALS_DAYS[1]*DAY_MS; writeAll(arr); }
  ...
  function downloadBlob(filename, dataStr){
    const blob=new Blob([dataStr],{type:'application/json;charset=utf-8'}); const url=URL.createObjectURL(blob);
  ...
  async function importSRSFile(file){
    ...
    try{
      ...
      storage.set(cards); updateReviewBadge();
  ...
  /* ---------- Stan gry ---------- */
  let deck=[], queue=[], current=null, answered=false, wasCorrect=null, firstTryCount=0, totalCards=0;
  ...
  function loadCard(card){
    ...
    answerInput.value=''; answerInput.focus(); updateProgress();
  }
  ...
  function startWithPairs(pairs, mode){
    ...
    gameArea.classList.remove('hidden'); finishArea.classList.add('hidden'); show(gameView); nextCard();
  }
  ...
  answerForm.addEventListener('submit', (e)=>{
    ...
    const userAns=answerInput.value; const ok=isCorrectAnswer(userAns, current.meaning);
...
wheel; koło
wrench; klucz
spark plug; świeca zapłonowa`; inputPairs.focus();
  });

  $('backBtn')?.addEventListener('click', ()=>{ show(setupView); updateReviewBadge(); });

  againBtn?.addEventListener('click', (e)=>{ e.stopPropagation(); deck.forEach(c=>c.failedBefore=false); queue=deck.slice(); firstTryCount=0; gameArea.classList.remove('hidden'); finishArea.classList.add('hidden'); nextCard(); });
  restartBtn?.addEventListener('click', (e)=>{ e.stopPropagation(); show(setupView); updateReviewBadge(); });
